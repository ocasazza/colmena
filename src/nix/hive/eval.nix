...
let
  ...
  in evalConfig {
    system =
      if profileType == "nix-darwin" then npkgs.stdenv.hostPlatform.system
      else if builtins.match ".*darwin.*" npkgs.stdenv.hostPlatform.system != null then "x86_64-linux"
      else npkgs.stdenv.hostPlatform.system;
